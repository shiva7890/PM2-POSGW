...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.coupon.DMASyncBrandCouponMINValidator;
...
public class DMASyncBrandCouponMINProcessor extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMASyncBrandCouponMINProcessor.class);

	public DMASyncBrandCouponMINProcessor()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = DMASyncBrandCouponMINValidator.isValidReqData(posGWAccountRechargeDTO);
		}
	
		logger.info("DMASyncBrandCouponMINProcessor.isValidReqData - Status:"+status);
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
           ...
           orderResponseDTO = dmTransactionDelegate.performAsyncBrandCpnRedeemption(orderRequestDTO);
		...
		/*	orderResponseDTO = new OrderResponseDTO();
			...
			orderResponseDTO.setOrderId(111); 
			orderResponseDTO.setCouponRedeemTransID("7777");*/
    ...
    public OrderRequestDTO packToServerDTO(POSGWMessageDTO posGWMessageDTO)
    {
     ...
     orderRequestDTO.setOperatorID(posGWAccountRechargeDTO.getOperatorID());
        ...
        orderRequestDTO.setProductCode(posGWAccountRechargeDTO.getProductCode());
        orderRequestDTO.setQuantity(posGWAccountRechargeDTO.getQuantity());
        //orderRequestDTO.setPinNo(posGWAccountRechargeDTO.getPinNumber());
        logger.info ("packToServerDTO(POSGWMessageDTO posGWMessageDTO)...Enddddd.");
	...
	public POSGWMessageDTO unpackISOMsg(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO)
	{
		...
		try
		{
           ...
           logger.info("DataUtil.getMappedOperatorId(processingCode)::"+DataUtil.getMappedOperatorId(processingCode));
			...
			logger.info("Amount from the ISOMsg::"+amount);	
			...
			logger.info("System Trace Audit Number::"+systemTraceAuditNum);	
			...
			logger.info("storemanagerId from the ISOMsg::"+storemanagerId);
			...
			logger.info("subagentId from the ISOMsg::"+subagentId);
			...
			logger.info("Validation ID from the ISOMsg::"+validationid);
...
//			newly added for costa rica 49
	...
	public ISOMsg packISOMsg(ISOMsg requestISOMsg,POSGWMessageDTO posMessageDTO)
	{
		...
		try
		{
			...
			{
				logger.info("DMASyncBrandCouponMINProcessor - packISOMsg()........");
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("DMASyncBrandCouponMINProcessor - execute()........");
	   ...
	   // End of AppMoniter Logging
			...
			logger.error("DMASyncBrandCouponMINProcessor - execute() at execute(): "+exp.getMessage());
			}
			catch(Exception ex){
				logger.error("DMASyncBrandCouponMINProcessor - execute()-Exception at execute(): "+exp.getMessage());
