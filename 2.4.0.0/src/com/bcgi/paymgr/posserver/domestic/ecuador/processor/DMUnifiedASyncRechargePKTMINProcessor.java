...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.DMUnifiedASyncRechargePKTMINValidator;
...
public class DMUnifiedASyncRechargePKTMINProcessor extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMUnifiedASyncRechargePKTMINProcessor.class);

	public DMUnifiedASyncRechargePKTMINProcessor()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			//status = DMUnifiedASyncRechargeMINValidator.isValidReqData(posGWAccountRechargeDTO);
			status = DMUnifiedASyncRechargePKTMINValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		/*else
		{
			status = ResponseCodeConstant.REQUEST_MESSAGE_NOT_FORMATTED_CORRECT;
		}*/
		logger.info("DMUnifiedASyncRechargePKTMINProcessor.isValidReqData - Status:"+status);
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			orderResponseDTO = dmTransactionDelegate.processUnifiedASyncMINRechargeTransaction(orderRequestDTO);
	...
	public POSGWMessageDTO unpackFromServerDTO(OrderResponseDTO orderResponseDTO)
	{
		...
		if (orderResponseDTO!= null)
		{
			...
			String posgwErrorId = com.bcgi.paymgr.posserver.domestic.ecuador.constant.PM_DMPOSGW_MapConstant.getPOSGWResponse(errorId);
    ...
    public OrderRequestDTO packToServerDTO(POSGWMessageDTO posGWMessageDTO)
    {
        ...
        orderRequestDTO.setReplenishmentChannelId(Integer.parseInt(posGWAccountRechargeDTO.getReplenishmentChannelId()));
...
//			newly added for costa rica 49
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("DMUnifiedASyncRechargePKTMINProcessor Processor - execute()........");
		...
		String resultMsg  = GatewayTransMoniterBuilder.getInstance().prepareMessage("","", requestISOMsg.getString(DMISOMessageFieldIDConstant.SYSTEM_TRACE_AUDIT_NUMBER), startCal, endCal,"Domestic Unified Async Recharge Transaction");
		
		if(posGWAccountRechargeDTO.getResponseCode().equalsIgnoreCase(ResponseCodeConstant.SUCCESS))
			AppMonitorWrapper.logEvent(AppMoniterEventConstants.UNIASYNCRECHARGE_GATEWAY_DOMPOS_SUCCESS, resultMsg, (int)roundTriptime, AppMoniterEventConstants.CARRIER_ID);
		else
			AppMonitorWrapper.logEvent(AppMoniterEventConstants.UNIASYNCRECHARGE_GATEWAY_DOMPOS_REJECTED, resultMsg, (int)roundTriptime, AppMoniterEventConstants.CARRIER_ID);
