...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.coupon.DMBrandCouponRedemptionValidator;
...
public class DMBrandCouponSubValidationProcessor extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMBrandCouponSubValidationProcessor.class);

	public DMBrandCouponSubValidationProcessor()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = DMBrandCouponRedemptionValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		/*else
		{
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		orderResponseDTO = dmTransactionDelegate.performBrandCpnAndSubscriberValidation(orderRequestDTO);
													 
			/*orderResponseDTO =new OrderResponseDTO();
			orderResponseDTO.setTransactionSucess(true);
			orderResponseDTO.setErrorId("0");
			orderResponseDTO.setValidationId(222); */
	...
	public POSGWMessageDTO unpackISOMsg(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO)
	{
		...
		try
		{
           ...
           logger.info("DataUtil.getMappedOperatorId(processingCode)::"+DataUtil.getMappedOperatorId(processingCode));
			...
			logger.info("Transaction Amount  from the ISOMsg::"+amount);
           	
			// 11 - System Trace Audit Number i.e RRN -Recharge Reference Number
			...
			logger.info("System Trace Audit Number  from the ISOMsg::"+systemTraceAuditNum);
			...
			logger.info("SalesPersonId  from the ISOMsg::"+storemanagerId);
            // 42- SubDistId
