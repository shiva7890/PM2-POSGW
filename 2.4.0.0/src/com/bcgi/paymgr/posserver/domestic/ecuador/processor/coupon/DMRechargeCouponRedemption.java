...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.coupon.DMCouponRechargeValidator;
...
public class DMRechargeCouponRedemption extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMRechargeCouponRedemption.class);

	public DMRechargeCouponRedemption()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = DMCouponRechargeValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		else
		{
			status = ResponseCodeConstant.REQUEST_MESSAGE_NOT_FORMATTED_CORRECT;
		}
		logger.info("DMCouponRechargeValidator.isValidReqData - Status:"+status);
	...
	public  POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		//	String subscriberId = reqTransDetDTO.getCustomerId();
			respTransDetDTO = rechargeDelegate.processRechargeCouponRedemption(reqTransDetDTO);
	...
	public POSGWMessageDTO processRejectedTransaction(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			//DMTransactionDelegate rechargeDelegate = new DMTransactionDelegate(ServerConfigConstant.RR_SYNCHRONOUS_CALL);
			DMTransactionDelegate rechargeDelegate = new DMTransactionDelegate(ServerConfigConstant.RR_SYNCHRONOUS_CALL, reqTransDetDTO.getFinIstutinalReferenceNumber());
...
//			newly added for costa rica 49
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("DMRechargeCouponRedemption  - execute()........");
		...
		try{
			...
			String resultMsg  = GatewayTransMoniterBuilder.getInstance().prepareMessage(posGWAccountRechargeDTO.getSubscriberId1(),posGWAccountRechargeDTO.getAuthorizationNumber(),posGWAccountRechargeDTO.getSystemTraceAuditNumber(), startCal, endCal,"Domestic Coupon Recharge Redeemption Transaction");
			if(posGWAccountRechargeDTO.getResponseCode().equalsIgnoreCase(ResponseCodeConstant.SUCCESS))
				AppMonitorWrapper.logEvent(AppMoniterEventConstants.COUPON_RECHARGE_GATEWAY_DOMPOS_SUCCESS, resultMsg, (int)roundTriptime, AppMoniterEventConstants.CARRIER_ID);
			else
				AppMonitorWrapper.logEvent(AppMoniterEventConstants.COUPON_RECHARGE_GATEWAY_DOMPOS_REJECTED, resultMsg, (int)roundTriptime, AppMoniterEventConstants.CARRIER_ID);
