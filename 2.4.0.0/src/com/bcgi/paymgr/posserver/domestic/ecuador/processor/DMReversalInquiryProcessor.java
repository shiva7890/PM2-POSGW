...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.DMReversalInquiryValidator;
...
public class DMReversalInquiryProcessor extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMReversalInquiryProcessor.class);

	public DMReversalInquiryProcessor()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = DMReversalInquiryValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		/*else
		{
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			orderResponseDTO = dmTransactionDelegate.processReversalInquiryTransaction(orderRequestDTO);
	...
	public POSGWMessageDTO processRejectedTransaction(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			//added by Rajender on 31/01/08  to avoid the txID for Reversal Inquiry for both synch and Asynch
    ...
    public OrderRequestDTO packToServerDTO(POSGWMessageDTO posGWMessageDTO)
    {
        ...
        //orderRequestDTO.setOrderId(Integer.parseInt(posGWAccountRechargeDTO.getOrderId()));
        ...
        // Added by Vamsi K on 23rd January'08: Asynchronous Phase II: Start -->
	...
	{
		POSGWAccountRechargeDTO posGWAccountRechargeDTO = new POSGWAccountRechargeDTO();
		posGWAccountRechargeDTO.setResponseCode(com.bcgi.paymgr.posserver.domestic.common.constant.ResponseCodeConstant.INITIAL_STATE);
		...
		try
		{
            ...
            String validationId = ""; // Added by Vamsi K on 23nd January'08: Phase II (Asynchronous)
			...
			// Added by Vamsi K on 23nd January'08: Phase II (Asynchronous): Start -->
			validationId = requestISOMsg.getString(DMISOMessageFieldIDConstant.VALIDATION_ID);
			System.out.println("VAMSI: After unpacking: "+ validationId);
			...
			posGWAccountRechargeDTO.setValidationId(validationId); // Added by Vamsi K on 23nd January'08
		}
		catch(Exception exp){
			...
			if (posMessageDTO != null)
			{
			    ...
			    if (responseCode != null)
			    {
			    	if (responseCode.equals(ResponseCodeConstant.SUCCESS))
			    	{
			    		...
			    		logger.debug("posgworderstatus in Reversal Inquiry:"+ posgworderstatus);
			    		logger.debug("orderstatus in Reversal Inquiry:"+ orderstatus);
