...
public class LRPRechargePKTMINProcessor extends DMBaseMessageProcessor{
	static Logger logger	= Logger.getLogger(LRPRechargePKTMINProcessor.class);

	public LRPRechargePKTMINProcessor()
	{
	...
	public POSGWMessageDTO unpackISOMsg(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		try{
           ...
           logger.info("DataUtil.getMappedOperatorId(processingCode)::"+DataUtil.getMappedOperatorId(processingCode));
			...
			String packetCode=requestISOMsg.getString(LRPISOMessageFieldIDConstant.PACKET_CODE);
			logger.info(" 46 packet Code  from the ISOMsg::"+packetCode);
			if (packetCode != null){
