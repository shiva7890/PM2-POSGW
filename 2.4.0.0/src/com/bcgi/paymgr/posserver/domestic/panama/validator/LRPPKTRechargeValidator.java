...
public class LRPPKTRechargeValidator {
	static Logger logger	= Logger.getLogger(LRPPKTRechargeValidator.class);
    	...
    	*/
    	...
    	System.out.println("packetCode :::::>>>>>"+packetCode);
    	 if(packetCode!= null  &&  packetCode.trim().length()>0){
    		 status = LRPReqMsgDataValidator.isValidPacketCode(packetCode,posGWAccountRechargeDTO.getProcessingCode());
    		 if (!status.equals(ResponseCodeConstant.INTERNAL_SUCCESS_STATUS)){
    				return status;
    			}
    	 }else{
