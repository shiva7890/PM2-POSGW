...
import com.bcgi.paymgr.posserver.domestic.ecuador.validator.coupon.DMASyncRechargeCpnMINValidator;
...
public class DMASyncRechargeCpnMINProcessor extends DMBaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(DMASyncRechargeCpnMINProcessor.class);

	public DMASyncRechargeCpnMINProcessor()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = DMASyncRechargeCpnMINValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		
		logger.info("DMASyncRechargeCpnMINProcessor.isValidReqData - Status:"+status);
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		orderResponseDTO = dmTransactionDelegate.performAsyncRechargeCpnRedeemption(orderRequestDTO);
			/*orderResponseDTO = new OrderResponseDTO();
			orderResponseDTO.setTransactionSucess(true);
			orderResponseDTO.setErrorId("0");
			orderResponseDTO.setOrderId(1234);
			//orderResponseDTO.setValidationId(143); 
			 orderResponseDTO.setCouponRedeemTransID("88888"); */
	...
	public POSGWMessageDTO processRejectedTransaction(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			DMTransactionDelegate rechargeDelegate = new DMTransactionDelegate(ServerConfigConstant.RR_ASYNCHRONOUS_CALL,reqTransDetDTO.getFinIstutinalReferenceNumber());
	...
	public POSGWMessageDTO unpackFromServerDTO(OrderResponseDTO orderResponseDTO)
	{
		...
		if (orderResponseDTO!= null)
		{
		    ...
		    String CoupRedeemTransID = orderResponseDTO.getCouponRedeemTransID(); 
		    logger.error("After Retrieving from Map - CoupRedeemTransID: "+CoupRedeemTransID);
		    posGWAccountRechargeDTO.setCouponRedeemTransId(CoupRedeemTransID);
	...
	public POSGWMessageDTO unpackISOMsg(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO)
	{
		...
		try
		{
           ...
           logger.info("DataUtil.getMappedOperatorId(processingCode)::"+DataUtil.getMappedOperatorId(processingCode));
			...
			logger.info("2-Field --subscriber id from the ISOMsg::"+subscriberId1);
			...
			posGWAccountRechargeDTO.setTransactionAmount(amount);
           	logger.info("4 -Field -- Transaction Amount"+amount);
			
			// 11 -- System Trace Audit Number (RRN -Recharge Reference Number)
            ...
            logger.info("11 -Field --System Trace Audit Number"+systemTraceAuditNum);
			...
			logger.info("systemTraceAuditNum::"+systemTraceAuditNum+"12 -Field --TransactionTime: "+transactionTime);
			...
			logger.info("systemTraceAuditNum::"+systemTraceAuditNum+"13-Field --TransactionDate: "+transactionDate);
			posGWAccountRechargeDTO.setTransactionDate(transactionDate);
			
			// 24 -- Network International ID i.e Distributor Id
			String distributorId = requestISOMsg.getString(DMISOMessageFieldIDConstant.DISTRIBUTOR_ID);
			logger.info("systemTraceAuditNum::"+systemTraceAuditNum+"24-Field --Distributor id from the ISOMsg::"+distributorId);
            ...
            logger.info("systemTraceAuditNum::"+systemTraceAuditNum+"41-Field --SalesPersonId"+storemanagerId);
			
			// 42 -- SubAgentId   i.e subDistributorId
			...
			posGWAccountRechargeDTO.setSubAgentId(subagentId);
			 logger.info("systemTraceAuditNum::"+systemTraceAuditNum+"42-Field --SubDistributorId"+subagentId);
			...
			posGWAccountRechargeDTO.setPinNumber(cpnCode);
			 logger.info("43-Field --SubDistributorId"+cpnCode);
			...
			logger.info("61-Field ---validationId"+validationid);
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("DMASyncRechargeCpnMINProcessor - execute()........");
