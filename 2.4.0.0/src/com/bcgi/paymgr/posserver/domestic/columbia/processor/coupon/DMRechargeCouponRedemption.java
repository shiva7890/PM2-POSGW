...
public class DMRechargeCouponRedemption extends DMBaseMessageProcessor
{
	...
	static Logger logger	= Logger.getLogger(DMRechargeCouponRedemption.class);
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = LRCRechargeValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		else
		{
...
//			 not modified, direclty using field 42
			...
			String couponPIN = requestISOMsg.getString(LRCISOMessageFieldIDConstant.PIN_NUMBER1);
			if (couponPIN != null){
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("Domestic Columbia DMRechargeCouponRedemption- execute()........"+posGWMessageDTO);
