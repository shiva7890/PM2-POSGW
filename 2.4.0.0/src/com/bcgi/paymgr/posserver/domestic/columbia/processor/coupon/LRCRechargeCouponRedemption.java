...
public class LRCRechargeCouponRedemption extends DMBaseMessageProcessor
{
	
	
	static Logger logger	= Logger.getLogger(LRCRechargeCouponRedemption.class);
	private String connectionId=null;

	public LRCRechargeCouponRedemption()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = LRCCouponRechargeValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		else
		{
	...
	public POSGWMessageDTO processMessage(POSGWMessageDTO reqPOSGWMessageDTO)
	{
		...
		try
		{
			...
			respTransDetDTO = rechargeDelegate.processCouponRechargeTransaction(reqTransDetDTO);
...
//			 not modified, direclty using field 42
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("LRCRechargeCouponRedemptionexecute()........"+posGWMessageDTO);
