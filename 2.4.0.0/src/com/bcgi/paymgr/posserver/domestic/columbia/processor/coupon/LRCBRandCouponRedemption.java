...
import com.bcgi.paymgr.posserver.domestic.columbia.validator.coupon.LRCBrandCouponValidator;
import com.bcgi.paymgr.posserver.domestic.columbia.validator.coupon.LRCCouponRechargeValidator;
...
public class LRCBRandCouponRedemption extends DMBaseMessageProcessor
{
	
	
	static Logger logger	= Logger.getLogger(LRCBRandCouponRedemption.class);
	private String connectionId=null;

	public LRCBRandCouponRedemption()
	{
	...
	public String isValidMessage(POSGWMessageDTO posGWMessageDTO)
	{
		...
		if (posGWMessageDTO != null)
		{
			...
			status = LRCBrandCouponValidator.isValidReqData(posGWAccountRechargeDTO);
		}
		else
		{
...
//			 not modified, direclty using field 42
			...
			String productcode  = requestISOMsg.getString(LRCISOMessageFieldIDConstant.PRODUCT_ID);
			...
			String quantity  = requestISOMsg.getString(LRCISOMessageFieldIDConstant.QUANTITY);
			...
			String couponPIN = requestISOMsg.getString(LRCISOMessageFieldIDConstant.PIN_NUMBER);
			if (couponPIN != null){
				couponPIN = couponPIN.trim();
			}
			
			posGWAccountRechargeDTO.setPinNumber(couponPIN);
			logger.debug(" 54 couponPIN from ISO Msg : "+couponPIN);
	...
	public ISOMsg execute(ISOMsg requestISOMsg,POSGWMessageDTO posGWMessageDTO){
		...
		logger.info("LRCBRandCouponRedemption()........"+posGWMessageDTO);
