...
public class PinPurchaseProcessor extends BaseMessageProcessor
{
	static org.apache.log4j.Logger logger	= org.apache.log4j.Logger.getLogger(PinPurchaseProcessor.class);
	public PinPurchaseProcessor() {
	...
	public POSMessageDTO processMessage(POSMessageDTO posMessageDTO)
	{
		...
		try
		{
			...
			posAccountRechargeDTO = rechargeDelegate.processPinPurchaseTransaction(posAccountRechargeDTO);
	...
	public ISOMsg packISOMsg(ISOMsg requestISOMsg,POSMessageDTO posMessageDTO)
	{
		...
		try
		{
			if (posMessageDTO != null)
			{
				...
				{
					requestISOMsg.set(ISOMessageFieldIDConstant.PIN_NUMBER,posAccountRechargeDTO.getPinNumber());
					requestISOMsg.set(ISOMessageFieldIDConstant.EXPIRY_DATE,posAccountRechargeDTO.getPinExpiryDate());
				}
	...
	public void updateSendMsgStatus(String transactionID,String status){
		...
		try{
			...
			exp.printStackTrace();
			logger.error("PinPurchaseProcessor-Exception at updateSendMsgStatus(): "+exp.getMessage());
